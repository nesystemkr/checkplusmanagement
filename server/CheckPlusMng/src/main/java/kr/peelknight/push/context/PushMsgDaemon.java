package kr.peelknight.push.context;
...
import kr.peelknight.messagequeue.context.ConsumerBaseDaemon;
import kr.peelknight.push.dao.PushDao;
import kr.peelknight.push.model.PS_Msg;
import kr.peelknight.push.model.PS_QueueItem;
...
public class PushMsgDaemon extends ConsumerBaseDaemon {
	...
	public void handleItem(Model item) {
		...
		if (pItem.getUserId() != null) {
			existUser = userDao.selectUserByUserId(pItem.getUserId());
		} else {
			existUser = userDao.selectUserByIdKey(pItem.getUserIdKey());
		}
		if (existUser == null) {
			return;
		}
	...
	public static void pushMessageDirect(String userId, String title, String message) {
		...
		CM_User existUser = userDao.selectUserByUserId(userId);
		if (existUser == null) {
			return;
		}
		List<CM_Device> devices = deviceDao.selectDevicesByUserIdKey(existUser.getIdKey());
