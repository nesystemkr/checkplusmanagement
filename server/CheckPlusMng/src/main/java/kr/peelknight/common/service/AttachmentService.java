...
public class AttachmentService {
	...
	public Response uploadAttachment(@FormDataParam("reqToken") String authToken,
		...
		try {
			...
			dao.insertAttachmentGroup(group);
...
//			CommonFunc.writeImage(resizeImage, monthDir + "/IMAGE_" + uuid.toString() + ".jpg", false);
	...
	public Response appendAttachment(@PathParam("groupIdKey") long groupIdKey,
		...
		try {
			...
			CM_AttachmentGroup existGroup = dao.selectAttachmentGroupByIdKey(groupIdKey);
			if (existGroup == null) {
			...
			dao.updateAttachmentGroup(existGroup);
	...
	public Response deleteAttachment(@PathParam("groupIdKey") long groupIdKey,
		...
		try {
			...
			CM_Attachment attachment = dao.selectAttachmentByIdKey(attachmentIdKey);
			if (attachment == null) {
				return ResponseUtil.getResponse(Status.NOT_FOUND);
			}
	...
	public boolean deleteAttachGroupFile(long groupIdKey, long attachmentIdKey, String authToken) {
		CM_AttachmentGroup group = dao.selectAttachmentGroupByIdKey(groupIdKey);
		if (group == null) {
			return false;
		}
		...
		if (attachmentIdKey > 0) {
			...
			List<CM_Attachment> list = dao.selectAttachments(groupIdKey);
			group.setGroupFileCount(list.size());
			dao.updateAttachmentGroup(group);
		} else {
			CM_Attachment attachment;
			List<CM_Attachment> list = dao.selectAttachments(groupIdKey);
			dao.deleteAttachmentGroup(list, group);
	...
	public Response selectAttachmentGroup(@PathParam("groupIdKey") long groupIdKey) {
		try {
			...
			group.setAttachments(dao.selectAttachments(groupIdKey));
			return ResponseUtil.getResponse((new ModelHandler<CM_AttachmentGroup>(CM_AttachmentGroup.class)).convertToJson(group));
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseUtil.internalError(e.getMessage());
