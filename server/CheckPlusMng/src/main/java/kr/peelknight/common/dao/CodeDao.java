...
public class CodeDao {
	...
	public List<CM_Code> selectCodes(int offset, int size) {
		...
		try {
			...
			return session.selectList("kr.peelknight.mybatis.codeMapper.selectCode", param);
		} finally {
			session.close();
		}
	}
	...
	public CM_Paging selectCodePaging() {
		...
		try {
			HashMap<String, Object> param = new HashMap<String, Object>();
			return session.selectOne("kr.peelknight.mybatis.codeMapper.selectCodePaging", param);
		} finally {
			session.close();
		}
	}
	...
	public CM_Code selectCodeByTypeNCode(String typeParam, String codeParam) {
		...
		try {
			...
			return session.selectOne("kr.peelknight.mybatis.codeMapper.selectCode", param);
		} finally {
			session.close();
		}
	}
	...
	public void insertCode(CM_Code code) {
		SqlSession session = factory.openSession();
		try {
			session.insert("kr.peelknight.mybatis.codeMapper.insertCode", code);
			session.commit();
		} finally {
			session.close();
		}
	...
	public void insertCodes(List<CM_Code> list) {
		...
		try {
			HashMap<String, Object> param = new HashMap<String, Object>();
			param.put("list", list);
			session.insert("kr.peelknight.mybatis.codeMapper.insertCodes", param);
			session.commit();
		} finally {
			session.close();
		}
	...
	public void updateCode(CM_Code code) {
		SqlSession session = factory.openSession();
		try {
			session.update("kr.peelknight.mybatis.codeMapper.updateCode", code);
			session.commit();
		} finally {
			session.close();
		}
	...
	public void deleteCode(CM_Code code) {
		SqlSession session = factory.openSession();
		try {
			session.delete("kr.peelknight.mybatis.codeMapper.deleteCode", code);
			session.commit();
		} finally {
			session.close();
		}
		...
		try {
			...
			for (int ii=0; ii<list.size(); ii++) {
				...
				if ("I".equals(item.getTouch())) {
					session.insert("kr.peelknight.mybatis.codeMapper.insertCode", item);
				} else if ("U".equals(item.getTouch())) {
					session.update("kr.peelknight.mybatis.codeMapper.updateCode", item);
				}
			}
			session.commit();
		} finally {
		...
		try {
			...
			for (int ii=0; ii<list.size(); ii++) {
				...
				if (item.getType() != null && !item.getType().isEmpty()) {
					session.delete("kr.peelknight.mybatis.codeMapper.deleteCode", item);
				}
			}
			session.commit();
		} finally {
