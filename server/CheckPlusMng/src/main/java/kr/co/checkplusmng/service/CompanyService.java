...
public class CompanyService {
	...
	public Response insert(MW_Company company) throws Exception {
		try {
			...
			MW_Company existCompany = dao.selectByCompanyId(null, company.getCompanyId());
	...
	public Response update(@PathParam("idKey") long idKey,
		...
		try {
			...
			MW_Company existCompany = dao.select(null, idKey);
			if (existCompany == null) {
			...
			return ResponseUtil.getResponse((new ModelHandler<MW_Company>(MW_Company.class)).convertToJson(existCompany));
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseUtil.internalError(e.getMessage());
		}
	...
	public Response one(@Context HttpServletRequest request,
		...
		try {
			...
			MW_Company existUser = dao.select(request.getSession(), idKey);
			if (existUser == null) {
				return ResponseUtil.getResponse(Status.NOT_FOUND);
			}
			return ResponseUtil.getResponse((new ModelHandler<MW_Company>(MW_Company.class)).convertToJson(existUser));
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseUtil.internalError(e.getMessage());
		}
